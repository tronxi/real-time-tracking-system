...
\subsection{Procolos de transmisión de vídeo en tiempo real}
...
\begin{itemize}
    \item \textbf{RTMP (Real-Time Messaging Protocol):} Desarrollado originalmente por Adobe para transmitir vídeo, audio y datos sobre TCP. Es un protocolo robusto con baja latencia, ideal para transmisiones en directo. Aunque fue diseñado para Flash, aún se utiliza ampliamente con servidores como Nginx RTMP o Wowza, y es compatible con servicios como YouTube o Facebook Live.

    \item \textbf{HLS (HTTP Live Streaming):} Protocolo desarrollado por Apple. Divide el vídeo en fragmentos pequeños (.ts) y los entrega a través de HTTP, permitiendo la adaptación dinámica a distintos anchos de banda (streaming adaptativo). Es altamente compatible con navegadores y dispositivos, pero tiene mayor latencia.

    \item \textbf{MPEG-DASH (Dynamic Adaptive Streaming over HTTP):} Estándar abierto similar a HLS, pero no propietario. Divide el contenido en segmentos y permite el streaming adaptativo según el ancho de banda disponible. Su adopción está creciendo, especialmente en sistemas modernos y plataformas abiertas.

    \item \textbf{WebRTC:} Protocolo diseñado para la comunicación en tiempo real entre navegadores, muy utilizado para videollamadas. Ofrece una latencia extremadamente baja, pero su configuración e implementación son más complejas en entornos embebidos o personalizados.
\end{itemize}
